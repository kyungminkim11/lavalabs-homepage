...
const openingMessages: Record<Locale, string> = {
...
function pickRoute(input: string) {
...
function buildDraft(input: string, route?: ServiceRoute) {
...
function buildReply(input: string, route?: ServiceRoute) {
  const normalized = normalize(input);
...
export function Chatbot({ locale }: { locale: Locale }) {
  ...
  const panelTitle = useMemo(() => {
    if (locale === "en") return "Lava Guide";
    if (locale === "jp") return "Lava Guide";
    return "Lava Guide";
  }, [locale]);
