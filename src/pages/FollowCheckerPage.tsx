...
export default function FollowCheckerPage() {
  ...
  return (
    <div className="app-shell follow-case-shell">
      <header className="site-header"><div className="nav-shell"><BrandMark href={text.home} /><div className="language-switcher"><a href="/projects/follow-checker/" aria-current={locale === "ko" ? "page" : undefined}>KO</a><a href="/en/projects/follow-checker/" aria-current={locale === "en" ? "page" : undefined}>EN</a><a href="/jp/projects/follow-checker/" aria-current={locale === "jp" ? "page" : undefined}>JP</a></div></div></header>
      <main id="main-content">
        <section className="section follow-case-hero"><div className="section-inner follow-case-grid"><div><p className="eyebrow">Lava Labs Web Tool</p><h1>{text.title}</h1><p className="follow-case-lead">{text.lead}</p><div className="button-row"><a className="button primary" href="https://unfollow.lavalabs.co.kr/" target="_blank" rel="noreferrer" onClick={() => trackEvent("case_study_cta", { locale, project: "Follow Checker" })}>{text.start}<ExternalLink aria-hidden="true" /></a><a className="button secondary" href={text.home}>Lava Labs</a></div></div><aside className="follow-case-panel"><img className="case-study-preview" src="/assets/project-previews/follow.svg" alt="맞팔체커 실제 서비스 화면" width="1280" height="800" /><span className="follow-case-badge">Public Beta</span><ul className="follow-case-points"><li><CheckCircle2 aria-hidden="true" /><span>Local browser analysis</span></li><li><ShieldCheck aria-hidden="true" /><span>Privacy-first workflow</span></li><li><FileArchive aria-hidden="true" /><span>Exported data file</span></li></ul></aside></div></section>
        <section className="section"><div className="section-inner"><div className="section-heading compact"><p className="eyebrow">Process</p><h2>{text.flow}</h2></div><div className="follow-case-grid-cards">{text.steps.map(([title, body]) => <article key={title}><h3>{title}</h3><p>{body}</p></article>)}</div></div></section>
        <section className="section follow-case-trust"><div className="section-inner"><div className="section-heading compact"><p className="eyebrow">Privacy & Boundaries</p><h2>{text.trust}</h2></div><div className="follow-case-boundaries"><article><h3>{locale === "ko" ? "제공하는 기능" : locale === "en" ? "Available" : "提供する機能"}</h3><ul>{text.yes.map((item) => <li key={item}>{item}</li>)}</ul></article><article><h3>{locale === "ko" ? "제공하지 않는 기능" : locale === "en" ? "Not provided" : "提供しない機能"}</h3><ul>{text.no.map((item) => <li key={item}>{item}</li>)}</ul></article></div><p className="follow-case-disclaimer">{text.notice}</p></div></section>
      </main>
