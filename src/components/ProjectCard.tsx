  ...
  return (
    <article className={`case-card project-${project.preview}`}>
      ...
      <div className="case-content">
        ...
        <p>{project.body}</p>
        {project.notice && <p className="case-notice"><ShieldCheck aria-hidden="true" /><span>{project.notice}</span></p>}
        <div className="tag-row">{project.tags.map((tag) => <span key={tag}>{tag}</span>)}</div>
        <div className="case-actions">
          <a href={project.href} target={external ? "_blank" : undefined} rel={external ? "noreferrer" : undefined} onClick={() => track("open_service")}>{project.cta}<ExternalLink aria-hidden="true" /></a>
          {project.detailHref && <a className="case-secondary-link" href={project.detailHref} onClick={() => track("open_case_study")}>{project.detailCta}<ChevronRight aria-hidden="true" /></a>}
        </div>
      </div>
    </article>
  );
